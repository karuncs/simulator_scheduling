 ...
 * 
 * Standard algorithm with extension
 */
public class SchedulingAlgo4 {
	/**
	...
	public SchedulingAlgo4(Consumer cons, ArrayList<StorageRequest> stoReq) {
		...
		if (cons instanceof BasicConsumerRing) {// if consumer is BC
			...
			// Note this gives priority to Basic consumers
	...
	private void loadSchedulingInRing(ArrayList<StorageRequest> StoReq,
		...
		Debug.debug(0, "loadSchedulingInRing called " +currNode.getBasicConsumer().id + " in basic consumer  "
				+ currNode.id);
